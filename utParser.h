...
TEST_F(ParserTest, parseVar) {
  ...
  Parser parser(scanner);
  vector<Term*> terms = parser.getArgs();
  Variable *v = dynamic_cast<Variable *>(terms[0]);
  ASSERT_EQ("_date", v->symbol());
}


// Given there is no string in scanner.
// When parser parses all terms via scanner.
...
TEST_F(ParserTest, ListAsStruct) {
  ...
  vector<Term*> terms = parser.getArgs();
  Struct *s = dynamic_cast<Struct *>(terms[0]);
  Number *n = dynamic_cast<Number *>(s->args(0));
  List *l = dynamic_cast<List *>(s->args(1));
  ASSERT_EQ(2, s->arity());
  ASSERT_EQ(".(1, [])", s->symbol());
  ASSERT_EQ("1", n->symbol());
  ASSERT_EQ("[]", l->symbol());
}
...
TEST_F(ParserTest, ListAsStruct2) {
  ...
  Struct *s = dynamic_cast<Struct *>(terms[0]);
  Number *n = dynamic_cast<Number *>(s->args(0));
  Struct *s2 = dynamic_cast<Struct *>(s->args(1));
  ASSERT_EQ(2, s->arity());
  ASSERT_EQ(".(2, .(1, []))", s->symbol());
  ASSERT_EQ("2", n->symbol());
  ASSERT_EQ(".(1, [])", s2->symbol());
}
